<template>
  <div class="cmb-bill-import">
    ...
    <el-card v-if="result" class="result-card" :class="{ error: result.error }">
      <template #header>
        <div class="card-header">
          ...
          <div style="float: right;">
            <el-button 
              type="success" 
              @click="exportData"
              :disabled="result.error || !result.data || result.data.length === 0"
            >
              导出数据
            </el-button>
            <el-button 
      ...
      <div v-if="!result.error" class="result-content">
        <!-- 导出信息展示 -->
        <el-card v-if="result.exportInfo" class="export-info-card" shadow="never">
          <template #header>
            <div class="clearfix">
              <span>导出记录信息</span>
            </div>
